...
searchButton.on("click", () => {
    ...
    if(query.val() !== " " ){   
        fetch(`https://api.unsplash.com/search/photos?query=${query.val()}&page=1&client_id=${ApiKey}`)
        .then( (Response) => {
...
$("body").on("click", (e) => {
   ...
   if( $(e.target).attr("id") === "paraImg" || $(e.target).attr("id") === "userId" || $(e.target).attr("id") === "para" ){
    ...
    $(".imgShowCase").empty()
    $(`
    <div class="img" style="background: url(${ $($(e.target).parent().children()[0]).attr("src") }); background-size: cover; background-position: center">
        
      </div>

      <div class="imgInfo">
      <img src=${$( $($(e.target).parent().children()[3]).children()[0]).attr("src") } alt=""> 
        <h4>${$($(e.target).parent().children()[2]).text() }</h4>
        <h3>${$($(e.target).parent().children()[1]).text().toUpperCase() } </h3>
        <p>Total Likes: <span id="totalLikes">${ $( $($(e.target).parent().children()[3]).children()[1]).text() }</span></p>
